...
import type { GatewayStatus } from "@/types/dashboard";
import Image from "next/image";
import { cva, type VariantProps } from "class-variance-authority";
import { cn } from "@/lib/utils";
import { Bullet } from "@/components/ui/bullet";

const gatewayStatusItemVariants = cva("border rounded-md ring-4", {
  variants: {
    variant: {
...
interface GatewayStatusItemProps
  extends VariantProps<typeof gatewayStatusItemVariants> {
  title: string;
  value: string;
  status: string;
  className?: string;
...
function GatewayStatusItem({
  title,
  value,
  status,
  variant,
  className,
}: GatewayStatusItemProps) {
  return (
    <div className={cn(gatewayStatusItemVariants({ variant }), className)}>
...
export default function GatewayStatusComponent({ statuses }: GatewayStatusProps) {
  return (
    <DashboardCard
      title="GATEWAY STATUS"
      intent="success"
      addon={<Badge variant="outline-success">OPERATIONAL</Badge>}
    >
